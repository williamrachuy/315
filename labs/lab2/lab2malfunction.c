...
float fmul(float a, float b){
   ...
   fltStructR.fraction = (int)(sMultiply(fltStructA.fraction, fltStructB.fraction) >> 32);   

   while (((fltStructR.fraction ^ fltStructR.sign) & 0x40000000) == 0) {
      fltStructR.fraction <<= 1;
   }
   
   //normalizeFloat(&fltStructR);

   printf("A: %.8f, S: 0x%08X, E: %d, F: 0x%08X\n", a, fltStructA.sign, fltStructA.exponent, fltStructA.fraction);
   printf("B: %.8f, S: 0x%08X, E: %d, F: 0x%08X\n", b, fltStructB.sign, fltStructB.exponent, fltStructB.fraction);
   printf("R: %.8f, S: 0x%08X, E: %d, F: 0x%08X\n", 0.0, fltStructR.sign, fltStructR.exponent, fltStructR.fraction);
