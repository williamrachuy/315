...
// Lookup table for _register names
const char _reg[33][6] = {"$zero", "$at", "$v0", "$v1", "$a0", "$a1", "$a2", 
...
void strDecoded(unsigned mem, char *retStr, unsigned PC) {
   ...
   if(type == 'F'){                                                  // If op code not recognized, output invalid instruction
      ...
      sprintf(retStr, "Opcode: 0x%02X, %c Type (%s)\n   Rs: 0x%02X (%s), Rt: 0x%02X (%s), Rd: 0x%02X (%s), Shamt: 0x%02X", 
         op, type, functStr, rs, _reg[rs], rt, _reg[rt], rd, _reg[rd], shamt);
   }
      ...
      sprintf(conCat, "   Rs: %d (%s), Rt: %d (%s), Imm: 0x%04X, SignExt: 0x%08X (%hd)", 
         rs, _reg[rs], rt, _reg[rt], imm, signExtend(imm), (short int)(imm));
      ...
      sprintf(retStr, "Op: 0x%02X, %c Type (%s)\n   JumpAddr: 0x%08X",        // output type and new address being jumped to
